function lickNoLick_Odor_rewsize
...
%         'GUI.FA_PunishFraction', 0;...
    ...
    S.RewardValveTimeS = GetValveTimes(S.GUI.RewardD, S.RewardValveCode);
     S.RewardValveTimeB = GetValveTimes(S.GUI.RewardB, S.RewardValveCode);
    %% Load Tables
    ...
    for currentTrial = 1:MaxTrials
       ...
       %% Assemble state matrix
        ...
        %%Sarah changed stuff here
        sma = AddState(sma,'Name', 'RewardS', ... % 4 possible outcome states: Reward (H2O + tone), Punish (air puff + tone), WNoise (white noise), Neutral (tone)
            'Timer', S.RewardValveTimeS,... %
            'StateChangeConditions', {'Tup', 'PostUsRecording'},...
            'OutputActions', {'ValveState', S.RewardValveCode, 'SoftCode', 1});
         sma = AddState(sma,'Name', 'RewardB', ... % 4 possible outcome states: Reward (H2O + tone), Punish (air puff + tone), WNoise (white noise), Neutral (tone)
            'Timer', S.RewardValveTimeB,... %
            'StateChangeConditions', {'Tup', 'PostUsRecording'},...
            'OutputActions', {'ValveState', S.RewardValveCode, 'SoftCode', 1});
        %%end
